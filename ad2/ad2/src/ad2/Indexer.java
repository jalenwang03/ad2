...
//    }
    ...
    public static void insertDoc(IndexWriter i, int item_id, String item_name,String description,String category,double buy_price,double latitude,double longitude){
	Document doc = new Document();
	if(description==null)
		description="";
//	doc.add(new TextField("doc_id", doc_id, Field.Store.YES));
...
//	doc.add(new DoubleField("lng",longitude,Field.Store.YES));
	...
	try {
	    ...
	    i.deleteAll();
	    List<itemBean> itemList=jdbc.rebuildIndexes("indexes");
	    for(int j=0;j<itemList.size();j++){
	    	insertDoc(i,itemList.get(j).getItem_ID(),itemList.get(j).getName(),itemList.get(j).getDescription(),itemList.get(j).getCatagory(),itemList.get(j).getBuy_price(),itemList.get(j).getLatitude(),itemList.get(j).getLongitude());
//	    	System.out.println(itemList.get(j).getItem_ID()+"    "+itemList.get(j).getDescription());
	    }
